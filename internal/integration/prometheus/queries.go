...
// aggregated at the deployment level using label_replace.
func BuildCPUUtilizationQuery(namespacesRegex string) string {
	return fmt.Sprintf(`(sum by (deployment, namespace) (
    label_replace(
...
and on(deployment) label_replace(
    kube_pod_container_resource_requests{namespace="%s", container!="", resource="cpu"},
    "deployment", "$1", "pod", "(.*)-[a-z0-9]+-[a-z0-9]+"
) * 0 + 1
or
...
)) * 100`, namespace, namespace, namespace, namespace, namespace)
}
